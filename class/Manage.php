<?php
        ...
        public function EditOrderElements($cl_id)
        {
            ...
            foreach ($product_list as $product_id)
            {
                $result .= '<select
                                ...
                                value="'. $product_id .'"
                            >
                                '. $display -> DisplayProducts($product_id) .'
                            </select>
        ...
        public function BasketRow($id, $counter)
        {
            ...
            echo '  <div>
                        <tr id="view">
                            ...
                            <td>' . $product -> Price() . '</td>
                            <td>
                                <button style="border-radius: 6px; background-color: red; border: none; color: white;" title="Usuń" type="submit" onclick="deleteBasketProd('. $counter .')">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-x-fill" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6.146-2.854a.5.5 0 0 1 .708 0L14 6.293l1.146-1.147a.5.5 0 0 1 .708.708L14.707 7l1.147 1.146a.5.5 0 0 1-.708.708L14 7.707l-1.146 1.147a.5.5 0 0 1-.708-.708L13.293 7l-1.147-1.146a.5.5 0 0 1 0-.708z"/>
                                    </svg>
                                </button>
            ...
            if (mysqli_num_rows($result) > 0)
            {
                while ($row = mysqli_fetch_assoc($result))
                {
                    ...
                    echo   '<div class="card-body" style="background-color: #101010;">
                                ...
                                <p class="card-text"><a class="a_view" href="sites/ProductSite.php?id='. $row['pr_id'] .'">'. $row['pr_title'] .'</a></p>
                            </div></div>';
    ...
    class SearchDisplay extends Manage
    {
        public function __construct()
        {
            parent::__construct();
